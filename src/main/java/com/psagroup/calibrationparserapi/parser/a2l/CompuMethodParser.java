...
public class CompuMethodParser extends A2LObjectParser {
    ...
    public void parse(BufferedReader reader, String currentLine) throws IOException {
        ...
        if (currentLine.replaceAll("\\s+", "").startsWith("/beginCOMPU_METHOD")) {
            ...
            while (!(currentLine = next(reader)).replaceAll("\\s+", "").startsWith("/endCOMPU_METHOD")) {
            ...
            try {
                ...
                System.out.println("CompuMethod:: " + compContent);
    ...
    private CompuMethod buildA2lObject(String compContent) {
        ...
        for(int i=0; i<compParts.length; i++){
            System.out.print(i + ":" + compParts[i] + " ; ");
        }
...
//        CompuMethod cc =  cb.build();
//        System.out.println("CHECK");
//        cc.printCompuMethodInfo();
//        return cc;
        return cb.build();
    }
